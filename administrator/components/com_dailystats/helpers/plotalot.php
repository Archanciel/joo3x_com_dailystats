...
function _gcFormatString($format)
{
	switch ($format)
		...
		case FORMAT_TIME_HHMM  : return "kkmm";
		case FORMAT_TIME_HHMMSS: return "kkmmss";
...
//
function _resolveQuery($query, $allow_any=false)
{
	$this->_resolveSpecialVariable($query);
	if ((strncasecmp($query,"select",6) != 0) and (!$allow_any))
		return $query;
...
{	
	$query = $this->chart_data->plot_array[$plot_number]['query'];
   	...
   	$this->datasets[$plot_number]['legend'] = $this->chart_data->plot_array[$plot_number]['legend'];
	if ($query == '')
		return true;
...
// Specify the plotting colours and line styles
	...
	foreach ($this->datasets as $p => $dataset)
		...
		if (!isset($this->chart_data->plot_array[0]['style']))
			$this->chart_data->plot_array[0]['style'] = PLOT_STYLE_NORMAL;
		switch ($this->chart_data->plot_array[0]['style'])
			{
...
// Y are the values
	...
	for ($r = 0; $r < $num_rows; $r++)
		...
		if ($add_values_to_labels)
			$x .= ' ('.$this->_formatValue($y,$this->chart_data->x_format).')';		// label includes our formatted value
...
//
function drawChart(&$chart_data)		// $chart_data is the data from a chart record
{
